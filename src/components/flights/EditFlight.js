...
class EditFlight extends Component {
  ...
  render() {
    ...
    const { loading, success, countries, id, destCountry, destAirport, orgCountry, orgAirport, date } = this.state
    return (
      <Dialog
        open={open}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', width: '100%', margin: '20px 0px' }}>
            <ThemeProvider theme={customPostTheme}>
              <Button
                ...
                disabled={loading}
                onClick={() => this.updateFlight(id, orgAirport, destAirport, date)}
              >
  ...
  async updateFlight(id, origin, destination, date) {
